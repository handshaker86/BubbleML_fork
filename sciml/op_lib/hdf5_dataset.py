    ...
    def reset(self):
        ...
        with h5py.File(self.filename, "r") as f:
            self._data["temp"] = torch.nan_to_num(
                torch.from_numpy(f["temperature"][:][self.steady_time :])
            )
            self._data["velx"] = torch.nan_to_num(
                torch.from_numpy(f["velx"][:][self.steady_time :])
            )
            self._data["vely"] = torch.nan_to_num(
                torch.from_numpy(f["vely"][:][self.steady_time :])
            )
            self._data["dfun"] = torch.nan_to_num(
                torch.from_numpy(f["dfun"][:][self.steady_time :])
            )
